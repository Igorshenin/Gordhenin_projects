...
from extensions import Exchanger, APIException
...
def help_bot(message: telebot.types.Message):
    ...
    bot.send_message(message.chat.id, text=Exchanger.get_text_help(), reply_markup=keyboard, parse_mode='MarkDown')
...
def convert(message: telebot.types.Message):
    if message.text.lower() == 'помощь':
        bot.send_message(message.chat.id, text=Exchanger.get_text_help(), parse_mode='MarkDown')
        ...
        try:
            ...
            if len(arguments) != 3:
                raise APIException('Нужно ввести через пробел две валюты и затем количество. Для помощи -> /help')
            quote, base, amount = arguments
            quote = quote.lower()
            base = base.lower()
            price, quote, base = Exchanger.get_price(quote, base, amount)
...
def get_page_symbols(page=0, max_symbols=22):
    symbols = sorted(Exchanger.get_symbols().items())
